...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Pressable, useColorScheme, Image, ActivityIndicator, TextInput, Modal, FlatList } from 'react-native';
...
const VacanciesScreen = () => {
  ...
  const [selectedCity, setSelectedCity] = useState(cities.length > 0 ? cities[0].title_az : 'All');
    ...
    try {
      ...
      const response = await axios.get('https://movieappi.onrender.com/vacancies', {
        params: {
          ...
          pageSize: pageSize,
          showFinished: true,
          city_id: selectedCity === 'All' ? undefined : cities.find(city => city.title_az === selectedCity)?.id,
        },
      });
      const data = response.data;
  ...
  useEffect(() => {
    Promise.all([fetchCities(), fetchCategories(), fetchCompanies()])
      .then(() => {
        setIsLoading(false);
        setSelectedCity(cities.length > 0 ? cities[0].title_az : 'All');
      })
      .catch(error => {
...
width:"100%",
    ...
    backgroundColor: isDarkMode ? '#333' : '#F4F9FD', 
      marginLeft:20,
    paddingHorizontal: 2,
    marginVertical: 30,
    marginHorizontal: 20,
    borderWidth: 2,
    borderColor: '#8843E1',
    borderRadius: 12,// Background color for dark mode
  }}
  onPress={() => setModalVisible(true)}
>
  <Text style={{ color: isDarkMode ? 'white' : 'black',left:20 }}>
    {selectedCity}
  </Text>
