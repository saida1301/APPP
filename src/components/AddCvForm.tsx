...
const AddCvForm = ({label, onPress}: any) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const requiredFields = [name, surname, position, father_name];
...
/>
  ...
  {isLoading ? (
    <ActivityIndicator color={isDarkMode.toString() === 'dark' ? 'white' : 'white'} size={'small'} style={{ flex: 1, justifyContent: 'center' }} />
  ) : (
    <Text style={{ textAlign: "center", alignSelf: "center", paddingVertical: 7, color: isDarkMode ? 'white' : 'white' }}>{t('add')}</Text>
  )}
</Pressable>
