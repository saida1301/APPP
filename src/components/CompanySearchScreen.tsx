...
const [blogs, setblogs] = useState([])
  ...
  const handleSearch = (query) => {
    ...
    blog.slug.toLowerCase().includes(query.toLowerCase())
      ...
      const filteredData = [...filteredCompanies, ...filteredVacancies, ...filteredTrainings, ...filteredBlogs];
  ...
  const navigateToDetails = (item) => {
    ...
    addToSearchHistory(name || position);
  };
  ...
  const renderSearchHistoryItem = (query) => (
    ...
    >
      ...
      <Icon
        ...
        size={20}
        color="#999"
        onPress={() => deleteFromSearchHistory(query)}
      />
    </Pressable>
  );
