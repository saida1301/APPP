...
console.log(user_id)
  ...
  const handleAddCompany = async () => {
    try {
      ...
      if (!selectedSectorId) {
        setSelectedSectorIdError('Sector is required');
      ...
      if (!address) {
        setAboutError('Address is required');
...
/>
        ...
        <Pressable onPress={handleImagePicker} style={[styles.input,    {backgroundColor: isDarkMode  ? "#1B1523" : '#F4F9FD'},]}>
          <Icon
            name="paperclip"
            size={24}
